...
export async function POST(req: Request, ctx: Params) {
  const limited = rateLimitOrThrow(req, { keyPrefix: `server-icon:${ctx.params.serverId}`, limit: 10, windowMs: 60_000 });
  ...
  if (file.size > 2 * 1024 * 1024) return NextResponse.json({ error: 'File too large' }, { status: 413 });
  ...
  try {
    await admin.storage.createBucket('server-icons', { public: true });
  } catch {}

  const path = `${ctx.params.serverId}.${ext}`;
  const buf = Buffer.from(await file.arrayBuffer());
  const upload = await admin.storage.from('server-icons').upload(path, buf, { upsert: true, contentType: file.type || 'image/png' });
  if (upload.error) return NextResponse.json({ error: upload.error.message }, { status: 500 });

  const { data } = admin.storage.from('server-icons').getPublicUrl(path);
  const updated = await prisma.server.update({
    where: { id: ctx.params.serverId },
    data: { iconUrl: data.publicUrl },
    select: { iconUrl: true }
  });
